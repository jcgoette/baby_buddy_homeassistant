...
class BabyBuddyChildSensor(BabyBuddySensor):
    ...
    def entity_picture(self) -> str | None:
        ...
        image: str | None = self.child.get(ATTR_PICTURE)
        if image and not image.startswith("http"):
            host = self.coordinator.config_entry.data[CONF_HOST]
            port = self.coordinator.config_entry.data[CONF_PORT]
            path = self.coordinator.config_entry.data[CONF_PATH]
            return f"{host}:{port}{path}{image}"
        return image
...
class BabyBuddyChildDataSensor(BabyBuddySensor):
    ...
    def name(self) -> str:
        ...
        return f"{self.child[ATTR_FIRST_NAME]} {self.child[ATTR_LAST_NAME]} last {sensor_type}".replace(
            "_", " "
        ).replace("-", " ").title()
    ...
    def native_unit_of_measurement(self) -> str | None:
        ...
        if unit is None:
            is_metric = self.coordinator.hass.config.units.temperature_unit == UnitOfTemperature.CELSIUS
            if self.entity_description.device_class == SensorDeviceClass.TEMPERATURE:
                unit = UnitOfTemperature.CELSIUS if is_metric else UnitOfTemperature.FAHRENHEIT
            elif self.entity_description.device_class == SensorDeviceClass.WEIGHT:
                unit = UnitOfMass.KILOGRAMS if is_metric else UnitOfMass.POUNDS
            elif self.entity_description.device_class == SensorDeviceClass.DISTANCE:
                unit = UnitOfLength.CENTIMETERS if is_metric else UnitOfLength.INCHES
            elif self.entity_description.device_class == SensorDeviceClass.VOLUME:
                unit = UnitOfVolume.MILLILITERS if is_metric else UnitOfVolume.FLUID_OUNCES
        return unit
